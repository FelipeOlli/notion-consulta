...
export function AlterdataContadoresList({ clienteId, tipo, titulo }: Props) {
  ...
  function iniciarEdicao(c: Contador) {
    setEditandoId(c.id);
    setEditLogin(c.login);
  ...
  async function salvarEdicao(id: string) {
    if (!editLogin.trim() || !editSenha.trim()) return;
    setSalvandoEdicao(true);
    await fetch(`/api/admin/alterdata/contadores/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ login: editLogin, senha: editSenha }),
    });
    await carregar();
    cancelarEdicao();
    ...
    <div className="space-y-3">
      ...
      {loading ? (
        ...
        <div className="space-y-1.5">
          {contadores.map((c) => (
            <div key={c.id} className="glass-card px-3 py-2 rounded-xl space-y-2">
              {editandoId === c.id ? (
                /* Modo edição */
                <div className="space-y-2">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <button
                      ...
                      disabled={salvandoEdicao || !editLogin.trim() || !editSenha.trim()}
                      className="text-xs px-3 py-1.5 rounded-lg border border-blue-500/30 text-blue-400 hover:text-blue-300 hover:border-blue-400/50 transition-colors disabled:opacity-40"
                    >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-1.5 shrink-0">
                    ...
                    <button
                      type="button"
                      onClick={() => setConfirmar({ acao: () => excluir(c.id), mensagem: `Excluir credencial de ${titulo}?` })}
